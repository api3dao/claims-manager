...
contract ClaimsManager {
    ...
    enum CoverageClaimStatus { None, Submitted, MediationOffered, Resolved }
    ...
    modifier onlyMediator() {
        require(isMediator[msg.sender], "Only API3 Service Coverage Claim Mediators");
        _;
    }
    ...
    constructor(address _api3Pool, address[] memory _arbitrators, address[] memory _mediators) {
    ...
    function provideMediationOffer(uint256 _amount, uint256 _claimCount, string calldata _mediatorEvidence) external onlyMediator() {
        ...
        require(_amount <= claim.claimedDamagesAmount, "Mediation offer must be <= claimed damages");
    ...
    function acceptMediationOffer(uint256 _claimCount) external {
        ...
        require(claim.status == CoverageClaimStatus.MediationOffered, "No open mediation offer");
